          ...
          <div class="col-md-6 divide-right">
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <span class="{$property|lower}"><strong>{l s='%label%:' sprintf=['%label%' => $property] d='Shop.Theme.Global'}</strong> {$property_value}</span><br>
                {/foreach}
                <p class="product-quantity">{l s='Quantity:' d='Shop.Theme.Checkout'}&nbsp;<strong>{$product.cart_quantity}</strong></p>
              </div>
            </div>
          </div>
          <div class="col-md-6">
            <div class="cart-content">
              ...
              {if $cart.subtotals.shipping.value}
                <p><strong>{l s='Shipping:' d='Shop.Theme.Checkout'}</strong> {$cart.subtotals.shipping.value} {hook h='displayCheckoutSubtotalDetails' subtotal=$cart.subtotals.shipping}</p>
              {/if}

              {if !$configuration.display_prices_tax_incl && $configuration.taxes_enabled}
                <p><span>{$cart.totals.total.label}&nbsp;{$cart.labels.tax_short}</span>&nbsp;<span>{$cart.totals.total.value}</span></p>
                <p><strong>{$cart.totals.total_including_tax.label}</strong>&nbsp;<span class="value">{$cart.totals.total_including_tax.value}</span></p>
              {else}
                <p><strong>{$cart.totals.total.label}:</strong> {$cart.totals.total.value} {if $configuration.taxes_enabled}{$cart.labels.tax_short}{/if}</p>
              {/if}
